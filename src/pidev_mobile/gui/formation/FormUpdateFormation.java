 ...
 * @author ASUS
 */
public class FormUpdateFormation extends BaseForm{
String imageF="";
    public FormUpdateFormation(Formation f,Form previous,Resources res) {
...
Image im=MyApplication.theme.getImage(f.getImageF()).scaled(300,400);
   limg.setIcon(im);
  ...
  image.addActionListener((ActionEvent e) -> {
            if (FileChooser.isAvailable()) {
                ...
                FileChooser.showOpenDialog( ".jpg, .jpeg,.png, .png/plain", (ActionEvent e2) -> {
                    if (e2 == null || e2.getSource() == null) {
                        limg.setText("No file was selected");
                        revalidate();
                        return;
                    }
    ...
    dateDD.setType(Display.PICKER_TYPE_DATE);
        try {
            dateDD.setDate(new SimpleDateFormat("yyyy-mm-dd").parse(new SimpleDateFormat("yyyy-mm-dd HH:mm:ss").format(f.getDateDebut())));
...
calendar.setTime(f.getDateDebut());   // assigns calendar to given date 
    ...
    dateFD.setType(Display.PICKER_TYPE_DATE);
      try {
            dateFD.setDate(new SimpleDateFormat("yyyy-mm-dd").parse(new SimpleDateFormat("yyyy-mm-dd HH:mm:ss").format(f.getDateFin())));
        } catch (ParseException ex) {
             System.out.println("erreur f date");
        }
    ...
    southButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                try {
                    ...
                    Formation F =new Formation(f.getId(),Lebelle.getText(), description.getText(), domaine.getText(), Lieu.getText(), datedeb, datefin, Float.parseFloat(montant.getText()), true, f.getLat(), f.getLng(), imageF);
