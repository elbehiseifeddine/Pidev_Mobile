...
public class FormationService {
    ...
    public static FormationService getInstance(){
     ...
     public boolean addFormation(Formation F,String type,int idu){
         ResultOK=false;
         String message = URLEncoder.encode(F.getLieu());
         String url=BASE_URL+"/addFormationMobile/?labelle="+F.getLabelle()+"&description="+F.getDescription()+"&domaine="+F.getDomaine()+"&lieu="+message+
                 "&dated="+F.getDateDebut()+"&datef="+F.getDateFin()+"&montant="+F.getMontant()+/*"/"+F.isEtat()+*/"&lat="+F.getLat()+"&lng="+F.getLng()+"&image="+F.getImageF()
                 +"&type="+type+"&idu="+idu;
          req.setUrl(url);
             
         System.out.println("msg="+message);
         System.out.println("hadha conetent lowel=>>>"+req.getContentType());
         req.setContentType("application/json; charset=UTF-8");
         System.out.println("hadha conetent theni=>>>"+req.getContentType());
  ...
  public boolean SupprimerFormation(Formation f){
   ...
   public boolean ModifierFormation(Formation F){
       ResultOK=false;
       String message = URLEncoder.encode(F.getLieu());
         String url=BASE_URL+"/updateFormationMobile/?id="+F.getId()+"&labelle="+F.getLabelle()+"&description="+F.getDescription()+"&domaine="+F.getDomaine()+"&lieu="+message+
                 "&dated="+F.getDateDebut()+"&datef="+F.getDateFin()+"&montant="+F.getMontant()+/*"/"+F.isEtat()+*/"&lat="+F.getLat()+"&lng="+F.getLng()+"&image="+F.getImageF();
          req.setUrl(url);
            req.setPost(false);
         req.addResponseListener(new ActionListener<NetworkEvent>() {
